...
def compute_pair_liquidity(
    ...
    Liquidity is approximated from yes_bid_amount + yes_ask_amount
    if available in snapshots.
    ...
    Returns:
        Average liquidity or None if data not available
    """
    ...
    for t in times_set:
        for market_id in [market_a_id, market_b_id]:
            ...
            if key in snapshot_index:
                ...
                # Try to get liquidity from bid/ask amounts
    ...
    return sum(liquidity_values) / len(liquidity_values)
...
def compute_pair_spread(
    ...
    Spread is computed as (ask - bid) / mid_price if available.
    ...
    Returns:
        Average spread (as decimal, e.g., 0.02 for 2%) or None if data not available
    """
    # Index snapshots by (market_id, time)
    snapshot_index: dict[tuple[str, str], dict[str, Any]] = {}
    for snap in snapshots:
    ...
    for t in times_set:
        for market_id in [market_a_id, market_b_id]:
            ...
            if key in snapshot_index:
                snap = snapshot_index[key]
                bid = snap.get("yes_bid", 0.0) or 0.0
                ask = snap.get("yes_ask", 0.0) or 0.0
    ...
    return sum(spread_values) / len(spread_values)
...
def apply_market_constraints(
    ...
    - min_liquidity: Minimum average liquidity required
    - max_spread: Maximum average spread allowed
    ...
    for pair in pairs:
        ...
        if pair.min_liquidity is not None:
            liquidity = compute_pair_liquidity(snapshots, pair.market_a_id, pair.market_b_id, times)
        ...
        if passed and pair.max_spread is not None:
            spread = compute_pair_spread(snapshots, pair.market_a_id, pair.market_b_id, times)
